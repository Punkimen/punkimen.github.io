...
export const stepAdaptive = (windowWidth) => {
  ...
  if (stepsWrapper && stepTitle && stepText) {
    if (windowWidth <= (768 * window.devicePixelRatio) && !stepsWrapper?.classList.contains('changed')) {
      addClass(stepsWrapper, 'changed')
      stepsWrapper.append(stepText)
    } else if (windowWidth > (768 * window.devicePixelRatio) && stepsWrapper?.classList.contains('changed')) {
...
export const circlesAdaptive = (windowWidth) => {
  ...
  if (cirles) {
    if (windowWidth <= (768 * window.devicePixelRatio) && !cirles.classList.contains('changed')) {
      addClass(cirles, 'changed')
      indItem.prepend(singleCircle)
      collItem.prepend(circlesWrapper)
    } else if (windowWidth > (768 * window.devicePixelRatio) && cirles.classList.contains('changed')) {
...
export const cardsAdaptive = (windowWidth) => {
  ...
  if (windowWidth <= (768 * window.devicePixelRatio) && !cardsWrapper.classList.contains('wrapper-row')) {
...
export const formAdaptive = (windowWidth) => {
  ...
  if (windowWidth <= (568 * window.devicePixelRatio) && !formField.classList.contains('adaptive')) {
    addClass(formField, 'adaptive')
    formText.after(formBtn)
  } else if ((windowWidth > (568 * window.devicePixelRatio) && formField.classList.contains('adaptive'))) {
