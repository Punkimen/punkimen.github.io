...
(function () {
  const checkboxes = [
    ...
    {
      text: "какой-текст на кирилце CRM",
      value: "",
      name: "checkbox",
    },
    {
  ...
  $('.show-modules').on('click', function () {
    if ($(this).hasClass('show')) {
      ...
      $(this).find('.show-modules__text').text('show 4 more modules')
    } else {
      $('.main__checkboxes-inner').addClass('show')
      $(this).find('.show-modules__text').text('hide 4 modules')
    }
...
(function () {
  ...
  if ($('.scroll-block').length > 0) {
    Scrollbar.init(document.querySelector('.scroll-block'));
  }
})();
...
$('[scroll-href]').on('click', function () {
  ...
  let href = $(this).attr('scroll-href');
  console.log(href);
...
const prevewImage = (input) => {
  if (input.files.length > 0) {
    const fileReader = new FileReader();

    fileReader.onload = (e) => {
      let img = input.closest('.image-add').querySelector('img');
...
// hero-types
if (document.querySelector('.hero-types__body')) {
  (function () {
    const types = [
      {
    ...
    const renderTypes = function () {
      types.forEach(({ text, href }) => {
        const typesInner = document.querySelector('.hero-types__body')
        const typeElem = document.createElement('a')

        typeElem.classList.add('hero-types__elem')
        typeElem.setAttribute('href', href)
        typeElem.textContent = text;
        typesInner.append(typeElem)
      })
    }

    $('.show-types').on('click', function () {
      if ($(this).hasClass('show')) {
        $('.hero-types__body').removeClass('show')
        $(this).find('.show-modules__text').text('show 2 more modules')
      } else {
        $('.hero-types__body').addClass('show')
        $(this).find('.show-modules__text').text('hide 2 modules')
      }
      $(this).toggleClass('show')
    });

    if ($('.hero-types__body').length > 0) {
      renderTypes();
    }
  })();
}
...
if (document.querySelector('#animate-text')) {
  (function () {
    let count = 0;
    const words = ['bank   ', 'credit union   ', 'fintech   ', 'retail   ', 'digital wallet   ', 'everyone   '];
    ...
    function writeOut(txtArr, curChar = 0) {
      ...
      setTimeout(
        () => {
          ...
          if (curChar == 0 && step !== 0 && count !== words.length - 1) {
            count++
            return writeOut(words[count].split(""), curChar + 1);
          }
          return writeOut(words[count].split(""), curChar + 1);
        },
        ...
        () => {
          ...
          if (elTxtArrLen == 0) {
            if (count !== words.length - 1) {
              return writeOut(words[count + 1].split(""), 0);
            } else {
              count = 0
              step = 0
              return writeOut(words[count].split(""), 0);
            }
          }
    ...
    const showWord = (arrWords, maxCount) => {
      if (count !== maxCount - 1) {
        document.querySelector('#animate-text').textContent = '';
        document.querySelector('#animate-text').textContent = arrWords[count];
        setTimeout(() => {
          count++
          return showWord(arrWords, arrWords.length)
        }, 400)
      } else {
        count = 0
        document.querySelector('#animate-text').textContent = '';
        writeOut(words[count].split(""));
      }
    }

    showWord(words, words.length)
  })();
}


// save local storages for
const saveEmail = (key, val) => {
...
const signUpBtn = document.querySelector('.hero-sign__btn');
if (signUpForm) {
  signUpBtn.addEventListener('click', (e) => {
    e.preventDefault();
    const email = signUpForm.querySelector('input').value;
    saveEmail('emailVal', email)
...
if (document.querySelector('#email')) {
  ...
  email.setAttribute('value', getLocalStorage('emailVal'))
}